...
% Get subject name
exp.subjID = 'trigger-test-allstimuli';
...
try
    ...
    % Wait Screen
    str = 'Press any key to start test!';
    drawAlignedText(p, str, p.wRect(4)/2, -2, 't', 'c')
...
%         Screen('Flip', p.whandle);
        ...
        for n = 1:80
            ...
            while toc < 1
                ...
                str = sprintf('run: %d / 12, trial: %d / 100', r, i);
                drawAlignedText(p, str, p.wRect(4)/2, -2, 't', 'c')
                Screen('Flip', p.whandle);
