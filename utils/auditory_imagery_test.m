...
% Instruction text
str = 'Now it is auditory imagery task. You will watch silent videos, and meawhile please imagine';
Screen('DrawText', p.whandle, str, p.xCenter-1000, p.yCenter-100, [0 0 0]);
str = 'the associated sound.';
...
for i = to_play
    ...
    videoID = trials(i);
    % Instruction text
    str = 'Press any button to play the silent video and imagine the sound';
    Screen('DrawText', p.whandle, str, p.xCenter-700, p.yCenter, [0 0 0]);
    Screen('Flip', p.whandle);
    
    KbWait([], 2);
    
    % Play the silent video
    ...
    str = 'Please rate the vividness of your auditory imagery';
    Screen('DrawText', p.whandle, str, p.xCenter-600, p.yCenter-400, [0 0 0]);
    str = '1 - No sound at all';
    ...
    Screen('DrawText', p.whandle, str, p.xCenter-400, p.yCenter+100, [0 0 0]);
    str = '5 - Perfectly clear and as vivid as normal hearing';
    Screen('DrawText', p.whandle, str, p.xCenter-400, p.yCenter+200, [0 0 0]);
    Screen('Flip', p.whandle);
    ...
    if keyCode(videoPlayKey)
        auditory_response(n_auditory,videoID) = 1;
    elseif keyCode(audioPlayKey)
        auditory_response(n_auditory,videoID) = 2;
    elseif keyCode(silentvideoPlayKey)
        auditory_response(n_auditory,videoID) = 3;
    elseif keyCode(KbName('4$'))
        auditory_response(n_auditory,videoID) = 4;
    elseif keyCode(KbName('5%'))
        auditory_response(n_auditory,videoID) = 5;
    end
